...
function mod.MenuInit(parent,list)
	...
	local group = GetActiveSpecGroup(true);

--	local id, name, description, icon, background, role = GetSpecializationInfo(group,true,false,INSPECTED_UNIT)
--	dump(id, name, description, icon, background, role)	
...
--	ex:SetBackgroundTexture("Interface\\TalentFrame\\"..background.."-");
	...
	for tier = 1, MAX_TALENT_TIERS do
		...
		local tierAvailable, selectedTalent, tierUnlockLevel = GetTalentTierInfo(tier,talentGroup,true,INSPECTED_UNIT);	-- doesn't appear to work correctly for inspects

		-- Set Tier Level Requirement
		group.levelText:SetText(tierUnlockLevel or tier);
